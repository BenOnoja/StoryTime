...
const PDFViewerComponent = () => {
    ...
    useEffect(() => {
        const checkPaymentStatus = async () => {
            try {
                const response = await fetch(
                    `/api/transaction-status?telegram_user_id=${user.id}&book_id=${selectedBookId}`
                );
        ...
        const fetchProgress = async () => {
            try {
                const response = await fetch(`/api/get-progress?telegram_user_id=${user.id}&book_id=${selectedBookId}`);
                const data = await response.json();
